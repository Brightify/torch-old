...
public class LoaderImpl<ENTITY> implements
    ...
    public LoaderImpl<ENTITY> or(EntityFilter filter) {
        EntityFilter entityFilter = new EntityFilter(null, new EntityFilter.OrFilterType());
        return this.<ENTITY>nextLoader(new LoaderType.FilterLoaderType(entityFilter)).filter(filter);
    }

    @Override
    public LoaderImpl<ENTITY> and(EntityFilter filter) {
        EntityFilter entityFilter = new EntityFilter(null, new EntityFilter.AndFilterType());
        return this.<ENTITY>nextLoader(new LoaderType.FilterLoaderType(entityFilter)).filter(filter);
    }
    ...
    public LoaderImpl<ENTITY> filter(EntityFilter nestedFilter) {
        return nextLoader(new LoaderType.FilterLoaderType(nestedFilter));
    }
